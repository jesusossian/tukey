...
GraphData::GraphData(const std::string &graphPath) : Data<PUNGraph>(new TUNGraph()) {
    if (std::filesystem::path(graphPath).extension() == ".edges" || std::filesystem::path(graphPath).extension() == ".txt") {
        try {
            ...
            std::map<NodeId, int> nodeMap;
            std::vector<std::pair<NodeId, NodeId>> edgs;
            std::string a, b;
            std::string line;
            ...
            while(std::getline(infile, line)){
                ...
                if (b.empty()){
                    ...
                    const NodeId src = std::stoi(a);
                    const NodeId dest = std::stoi(b);
                    nodeSet.insert(src);
                    nodeSet.insert(dest);
                    edgs.emplace_back(std::pair<NodeId, NodeId>(src, dest));
                }
            }
            ...
            for (auto edge : edgs) {
                this->graph()->AddEdge(nodeMap[edge.first], nodeMap[edge.second]);
            }
    ...
    this->graphDataInfo = GraphDataChangeable(this->get_graph());
    this->maxDegree = (int) this->graphDataInfo.neighborList.size();
}
...
GraphData::GraphData(const std::string& graphPath, const std::string& labelPath) : GraphData(graphPath){
    ...
    this->graphDataInfo = GraphDataChangeable(this->get_graph());
    this->maxDegree = this->graphDataInfo.neighborList.size();
}
...
GraphData::GraphData(const GraphData &other)  : Data<PUNGraph>(other){
    this->graphDataInfo = other.graphDataInfo;
    this->graphType = other.graphType;
}

void GraphData::print() const {
...
void GraphData::UpdateSingletons(const std::string &graphPath) {
    ...
    if (extension == ".edges" || extension == ".txt") {
        ...
        if (graphData.nodes() != graphData.get_graph()->GetMxNId()){
            throw std::range_error("Nodes " + std::to_string(graphData.nodes()) + " and Ids (" + std::to_string(graphData.get_graph()->GetMxNId()) + ") do not fit for the graph! " +
                                           graphData.GetName() + "\n");
        }
    }
...
void GraphData::init(int size) {
    this->graphDataInfo = GraphDataChangeable(this->get_graph());
//    this->DistanceList = std::vector<int>(this->get_graph()->GetNodes(), -2);
//    this->ContainmentList = std::vector<int>(this->get_graph()->GetNodes(), -1);
//    this->PredecessorsList = std::vector<std::vector<int>>(this->get_graph()->GetNodes(), std::vector<int>());
//    GraphFunctions::generateNeighborVector(this->get_graph(), this->neighborList);
    maxDegree = (int) this->graphDataInfo.neighborList.size();
}
...
GraphDataChangeable::GraphDataChangeable(const PUNGraph &graph) {
        this->distances = std::vector<int>(graph->GetNodes(), -2);
        this->containmentList = std::vector<int>(graph->GetNodes(), -1);
        this->predecessors = std::vector<std::vector<int>>(graph->GetNodes(), std::vector<int>());
        GraphFunctions::generateNeighborVector(graph, this->neighborList);
}

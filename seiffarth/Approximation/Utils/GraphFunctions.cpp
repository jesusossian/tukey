...
void GraphFunctions::bfsSubtree(const PUNGraph &undirectedGraph, GraphData &subTree, std::vector<NodeId> &neighborIds,
    ...
    ++subTree.graphDataInfo.Id;
    subTree.graphDataInfo.containmentList[rootNodeId] = subTree.graphDataInfo.Id;
    std::deque<NodeId> CurrentNodes;
    CurrentNodes.push_back(rootNodeId);
    while (!CurrentNodes.empty()){
        NodeId NextNodeId = CurrentNodes.back();
        ...
        for (NodeId i = 0; i < degree; ++i) {
            ...
            if (subTree.graphDataInfo.containmentList[NeighborNodeId] != subTree.graphDataInfo.Id){
                CurrentNodes.push_front(NeighborNodeId);
                subTree.graph()->AddEdge(NextNodeId, NeighborNodeId);
                subTree.graphDataInfo.containmentList[NeighborNodeId] = subTree.graphDataInfo.Id;
            }
        }
...
void GraphFunctions::GetLargestComponent(GraphData &data) {
    ...
    if (ConComps.Len() == 1) {
        ...
        PUNGraph biggestComponentGraph = TSnap::GetSubGraph(data.get_graph(), biggestComponent->NIdV, true);
        data = GraphData(biggestComponentGraph);
        data.SetName(data.GetName() + "_component");
    }
}
...
void GraphFunctions::analyse_graph(const PUNGraph& graph, const std::string& name, bool component_analysis, FileEvaluation* fileEvaluation) {
    ...
    std::cout << "\tDegree distribution: " << StaticFunctionsLib::printMap(degree_distribution) << std::endl;
    ...
    if (component_analysis) {
        ...
        for (int i = 0; i < conComponents.Len(); ++i) {
            ...
            std::cout << StaticFunctionsLib::printMap(degree_distribution) << std::endl;
            std::cout << std::endl;
        }
        std::cout << std::endl;
    }
    ...
    for (int j = 0; j < samples; ++j) {
        ...
        if (!save_samples || SmallGraph(graph)) {
            ...
            if (clear) {
                ...
                subgraph_data->graphDataInfo.containmentList.clear();
                subgraph_data->graphDataInfo.distances.clear();
                subgraph_data->graphDataInfo.neighborList.clear();
                subgraph_data->graphDataInfo.predecessors.clear();
            }
        }
...
void GraphFunctions::GetCoreGraphStats(GraphData &graph, TIntV &coreNodes, double runtime, const std::string& core_name, FileEvaluation* fileEvaluation) {
    ...
    std::cout << "\tOverlap degree distribution: " << StaticFunctionsLib::printMap(degree_distribution) << std::endl;
    std::cout << std::endl;
...
void GraphFunctions::get_graph_size(const std::string &path, int& nodes, int& edges) {
    ...
    In.read((char*) (&nodes), sizeof(int));
    In.read((char*) (&edges), sizeof(int));
    In.close();
}

void GraphFunctions::get_graph_paths(CoreCalcParams& params, std::vector<std::tuple<std::string, int>> &paths_sizes) {
...
#pragma omp parallel for default(none) shared(stripped_paths, paths_sizes, params) schedule(dynamic,1)
    for (auto const &stripped_path: stripped_paths) {
        std::string graph_path = stripped_path + ".bin";
        int nodes, edges;
        get_graph_size(graph_path, nodes, edges);
        if (nodes < params.max_nodes && edges < params.max_edges && nodes > params.min_nodes && edges > params.min_edges) {
#pragma omp critical
            paths_sizes.emplace_back(std::tuple<std::string, int>(stripped_path + ".bin", edges));
        }
    }
    std::sort(paths_sizes.begin(), paths_sizes.end(), StaticFunctionsLib::sortbysecond);
}

void GraphFunctions::get_graph_paths(GraphParams& params, std::vector<std::tuple<std::string, int>> &paths_sizes) {
    std::vector<std::string> stripped_paths;
    for (const auto &entry: std::filesystem::recursive_directory_iterator(params.in_path)) {
        std::string stripped_path =
                entry.path().parent_path().string() + "/" + entry.path().stem().string();
...
void GraphFunctions::get_graph_paths(CoreApproxApproxParams& params, std::vector<std::tuple<std::string, int>> &paths_sizes) {
    std::vector<std::string> stripped_paths;
...
void GraphFunctions::get_core(GraphData& graph, int generator_size, int core_iterations, int seed, std::vector<int>& intersection_loss, TIntV& coreNodes, double& runtime, double& avg_preclosure_steps, bool print){
    GraphClosureSP gc;
    ...
    for (int i = 0; i < core_iterations; ++i) {
        ...
        avg_preclosure_steps += closureParameters.preclosure_depth;
        ...
        if (print) {
            std::cout << "Iteration " << std::to_string(i) << "of graph " << graph.GetName() << "finished after "
                      << std::to_string(((double) std::chrono::duration_cast<std::chrono::microseconds>(
                              std::chrono::high_resolution_clock::now() - start).count() / 1000000.0)) << "seconds"
                      << std::endl;
        }
    }
    coreNodes.Clr();
...
void GraphFunctions::get_core(GraphData& graph, int generator_size, int core_iterations, int seed, TIntV& coreNodes, bool print) {
    std::vector<int> intersection_loss;
    double avg_preclosure_steps;
    double runtime;
    GraphFunctions::get_core(graph, generator_size, core_iterations, seed, intersection_loss, coreNodes, runtime, avg_preclosure_steps, print);
}

void GraphFunctions::BFSDistances(const PUNGraph &graph, NodeId root, std::vector<int> &distances) {
    distances.clear();
    distances.resize(graph->GetNodes(), -1);
    std::vector<bool> visitedNodes = std::vector<bool>(graph->GetNodes(), false);
    ...
    while (!nodes.empty()){
        ...
        nodes.pop_back();
        for (NodeId i = 0; i < graph->GetNI(currentNode).GetDeg(); ++i) {
            NodeId neighbor = graph->GetNI(currentNode).GetNbrNId(i);

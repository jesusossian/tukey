...
public:
    ...
    void approx_closure(const GraphData& graph, std::vector<OuterplanarGraphData> &samples, int& sample_number, ClosureParameters &parameters, std::vector<int> &approximation,
                        double& closure_time, bool load = false, ClosureFunction closure = nullptr);
    /// Increments the closed set closureParameters.closure by the element newNode and calculates the new closure
    /// \param data input graph
    /// \param closureParameters defines the closed set
    /// \param newNode new node which is added
    /// \return true if the new closure does not contain closureParameters.forbidden_elements, false otherwise
    bool incremental_closure(GraphData& data, ClosureParameters& closureParameters, NodeId newNode);


private:
    /// Calculates the preclosure i.e. all nodes on shortest paths between node and nodeSet
    /// \param graph input graph
    /// \param node some node in the graph
    /// \param nodeSet some node set in the graph
    /// \param forbidden_elements forbidden elements which
    /// \param output_intersects_forbidden true if the output intersects the forbidden elements, false o/w
    /// \param new_elements elements added in the preclosure step not beeing in nodeSet
    /// \return false if the output intersects the forbidden elements, true o/w
    bool pre_closure(GraphData& graph, NodeId node, std::set<NodeId>& nodeSet, const std::vector<std::set<NodeId>*>& forbidden_elements, std::set<NodeId>& new_elements);
